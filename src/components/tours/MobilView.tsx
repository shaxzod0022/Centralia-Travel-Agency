...
const Overview = ({ tourOption, addOns = [], generalInfo }: OverviewProps) => {
  ...
  // Mobile uchun Booking Section
  const MobileBookingSection = () => {
    if (!selectedDate || !showOptions) return null;

    return (
      <div className="xl:hidden space-y-4">
        {/* Tur Options - Mobile uchun */}
        <div className="space-y-4 animate-fadeIn">
          {(tourOption?.options || [])
            ...
            .map((item) => (
              <div
                key={item.id}
                className={`cursor-pointer shadow-lg rounded-xl p-6 space-y-4 transition-all duration-200 border-2 ${
              ...
              >
                <div className={`${styles.flexBetween} w-full items-start`}>
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`bg-[#6EBB2D] font-semibold !text-white ${styles.p} px-3 py-1 rounded-lg capitalize`}
                    >
                      {item.type}
                    </span>
                  </div>
        ...
        {/* Add-Ons Selector */}
        {selectedOption && selectedDate && addOns.length > 0 && (
          <AddOnsSelector
            addOns={addOns}
        ...
        {selectedDate && (
          <div className="space-y-4">
              ...
              {Object.keys(selectedAddOns).length > 0 && (
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-700">{t("addOnsPrice")}</span>
                    <span className="text-lg font-bold text-[#6EBB2D]">
                      ${calculateAddOnsTotal()}
                    </span>
                  </div>
                  {seasonalPrice && basePrice && seasonalPrice >= basePrice ? (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-700">{t("tourPrice")}</span>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xl font-bold text-gray-900">
                        ${seasonalPrice}{" "}
                        <s className="text-sm text-gray-400">${basePrice}</s>
                      </span>
                    </div>
                  ) : (
                    <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="w-full bg-[#6EBB2D] active:bg-[#74c72f] hover:bg-[#5da825] text-white font-bold text-md p-2 rounded-md transition-colors"
            >
              {t("bookNow")}{" "}
            ...
            <button
              ...
              className={`w-full ${isInCart ? "bg-orange-500" : "bg-[#056D50]"} text-white font-semibold text-md p-2 rounded-md transition-colors`}
            >
  ...
  // Calculate starting price (min price) for "From $XXX"
  const getMinPrice = () => {
    if (!tourOption?.options) return 0;

    const prices = tourOption.options.map((opt) => {
      if (opt.pricing.mode === "PER_PERSON") {
        ...
        return opt.pricing.personRanges[0]?.price || Infinity;
      }
    });
  ...
  // Visibility logic for sticky bar
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Show sticky bar ONLY if the calendar is completely below the viewport
      // (User hasn't reached it yet)
      // rect.top > window.innerHeight means the top of the calendar is below the bottom of the screen.
      // We can add a small offset (e.g., 50px) to trigger it slightly earlier or later if needed.
      const isBeforeCalendar = rect.top > window.innerHeight;

      setShowStickyBar(isBeforeCalendar);
    };
  ...
  const StickyAction = () => {
    ...
    return (
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-40 xl:hidden flex justify-between items-center animate-slideUp">
        <div className="flex flex-col">
          <span className="text-xs text-gray-500 uppercase font-medium">
            {t("startingFrom")}
          </span>
          <span className="text-xl font-bold text-[#056D50]">${minPrice}</span>
        </div>
        <button
          onClick={scrollToCalendar}
          className="bg-[#6EBB2D] active:bg-[#74c72f] hover:bg-[#5da825] text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all"
        >
          {t("checkAvailability")}
        </button>
      </div>
  ...
  return (
    <>
      <div className={`${styles.paddingCont} space-y-6 xl:hidden block`}>
        {/* Description Section */}
        <div className="space-y-4">
          <h2 className={`${styles.h2} !text-[20px]`}>{t("enjoy")}</h2>
        ...
        <div className="space-y-4">
          <h2 className={`${styles.h2} !text-[20px]`}>{t("incExc")}</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6 mt-8">
            ...
            <ul className="space-y-1">
              {generalInfo.tourIncluded?.map((item, idx) => (
                <li
                  key={idx}
                  className={`flex items-start gap-1 ${styles.span}`}
                >
                  <Check className="w-4 shrink-0 text-[#6EBB2D]" />
            ...
            <ul className="space-y-1">
              {generalInfo.tourExcluded?.map((item, idx) => (
                <li
                  ...
                  className={`flex items-start gap-1 ${styles.span}`}
                >
                  <X className="w-4 shrink-0 text-red-500" />
                  <span className="text-gray-700">{item}</span>
                </li>
              ))}
            </ul>
        ...
        {generalInfo.destinations && generalInfo.destinations.length > 0 && (
          <div className="space-y-4">
            <h2 className={`${styles.h2} !text-[20px]`}>{t("destination")}</h2>
            <div className="flex flex-wrap gap-3">
              {generalInfo.destinations?.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-1 border-2 border-[#6EBB2D] px-2 py-1 rounded-full bg-white hover:bg-gray-50 transition-colors text-sm"
                >
                  <MapPin className="w-4 text-[#6EBB2D]" />
        ...
        {generalInfo.highlights && generalInfo.highlights.length > 0 && (
          <div className="space-y-4">
            <h2 className={`${styles.h2} !text-[20px]`}>{t("highlights")}</h2>
            <div className="space-y-2">
              {generalInfo.highlights?.map((item, idx) => (
                <div key={idx} className="flex items-start gap-1">
                  <span
                    className={`${styles.flexCenter} shrink-0 w-6 h-6 bg-[#E6F4EA] rounded-full `}
                  >
                    <Check className="w-4 text-[#6EBB2D]" />
                  </span>
        ...
        {generalInfo.tourIncluded && generalInfo.tourIncluded.length !== 0 && (
        ...
        {generalInfo.tourMapLink && (
          <div className="space-y-4">
            <h2 className={`${styles.h2} !text-[20px]`}>{t("map")}</h2>
        ...
        {generalInfo.faqs && generalInfo.faqs.length !== 0 && (
          <>
            <h2 className={`${styles.h2} !text-[20px] mb-2`}>{t("faqs")}</h2>
            <Faqs data={generalInfo.faqs} />
          </>
        )}

        <div id="calendar" className="shadow-lg rounded-2xl">
          <GlobalCalendar
